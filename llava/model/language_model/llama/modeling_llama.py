...
def load_balancing_loss_func(
    ...
    if isinstance(gate_logits, tuple):
        ...
        concatenated_gate_logits = torch.cat([layer_gate.view(-1,num_experts).to(compute_device) for layer_gate in gate_logits], dim=0)
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, device=None):
        ...
        for i in range(len(self.base)):
            inv_freq = 1.0 / (self.base[i] ** (torch.arange(0, self.dim, 2).float().to(device) / self.dim))
            inv_freqs.append(inv_freq)
        inv_freq = torch.stack(inv_freqs)
        
        # inv_freq = 1.0/ (torch.tensor(self.base,device=device).unsqueeze(1) ** (torch.arange(0, self.dim, 2).to(device)/self.dim))
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        freqs = torch.einsum("i,jk->jik", t, self.inv_freq)
...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        if seq_len > self.max_position_embeddings:
            ...
            base = torch.tensor(self.base,device=device)* (
                (self.scaling_factor * seq_len / self.max_position_embeddings) - (self.scaling_factor - 1)
            ) ** (self.dim / (self.dim - 2))
            inv_freq = 1.0/ (base.unsqueeze(1) ** (torch.arange(0, self.dim, 2).to(device)/self.dim))
            self.register_buffer("inv_freq", inv_freq, persistent=False)
...
def apply_rotary_pos_emb_q(q, cos, sin,dim=2):
    cos = cos.unsqueeze(1).unsqueeze(dim)
    sin = sin.unsqueeze(1).unsqueeze(dim)
...
def repeat_kv_moe(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    """
    This is the equivalent of torch.repeat_interleave(x, dim=1, repeats=n_rep). The hidden states go from (batch,
    num_key_value_heads, seqlen, head_dim) to (batch, num_attention_heads, seqlen, head_dim)
    """
    ...
    def forward(
        ...
        if output_router_logits:
            aux_loss = load_balancing_loss_func(
                outputs.router_logits if return_dict else outputs[-1], self.num_experts, self.topk
            )
            if labels is not None:
